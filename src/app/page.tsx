...
export default function LandingPage() { 
  ...
  useEffect(() => {
    ...
    let barAnimationInterval: ReturnType<typeof setInterval> | null = null;
  ...
  return (
    <>
      <LandingPageNavigation /> {/* Added the new navigation bar */}
      <section
        className="relative min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-50 via-white to-teal-50 dark:from-slate-900 dark:via-slate-800 dark:to-teal-950 overflow-hidden pt-16" // Adjusted pt if nav is sticky
        data-section-view="hero"
      >
        <Particles
          className="absolute inset-0 z-0"
          quantity={50}
          color="#46B3AC" // A teal color, adjust as needed
          ease={50}
          staticity={50} // Make particles less reactive to mouse
        ...
        <div className="absolute inset-0 opacity-20 dark:opacity-30">
          ...
          <div className="absolute -bottom-60 left-1/2 -translate-x-1/2 w-[80rem] h-96 bg-gradient-to-r from-orange-400 to-red-400 rounded-full mix-blend-multiply filter blur-3xl opacity-30 dark:opacity-50 animate-pulse animation-delay-1500 z-0"></div>
          <div className="absolute -bottom-80 left-1/4 w-[70rem] h-80 bg-gradient-to-r from-purple-400 to-pink-400 rounded-full mix-blend-multiply filter blur-3xl opacity-30 dark:opacity-50 animate-pulse animation-delay-2000 z-0"></div>
          <div className="absolute -bottom-70 right-1/4 w-[90rem] h-80 bg-gradient-to-r from-blue-400 to-teal-400 rounded-full mix-blend-multiply filter blur-3xl opacity-30 dark:opacity-50 animate-pulse animation-delay-2500 z-0"></div>
        </div>
          ...
          <div className="max-w-6xl mx-auto">
                  ...
                  <div
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-full border transition-all duration-500 ${
                      index === currentFeature
                        ? 'bg-teal-100 dark:bg-teal-800/50 border-teal-300 dark:border-teal-600 text-teal-700 dark:text-teal-200 scale-105'
                        : 'bg-white/50 dark:bg-slate-700/30 border-gray-200 dark:border-slate-600 text-gray-500 dark:text-gray-400 scale-95'
                    }`}
                  >
            ...
            <div className={`flex flex-col sm:flex-row gap-4 justify-center items-center mb-12 transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'} animation-delay-1000`}>
              ...
              <Button
                size="lg"
                variant="outline"
                className="border-primary text-primary dark:border-sky-400 dark:text-sky-300 hover:bg-primary/10 dark:hover:bg-sky-400/10 px-8 py-4 text-lg font-semibold rounded-lg"
            ...
            >
              <div className="bg-slate-800 rounded-2xl shadow-2xl max-w-7xl mx-auto hover:shadow-3xl transition-all duration-500 group overflow-hidden moving-dashboard-border">

                <div className="bg-slate-800 rounded-t-2xl overflow-hidden relative">
                  ...
                  <div className="flex h-[650px]">
                    ...
                    <div className="flex-1 p-6 space-y-6 bg-slate-700"> 
                        ...
                        <div className="col-span-2">
                          <div className={`bg-slate-700 rounded-lg p-4 h-48 shadow-[-5px_-5px_10px_rgba(100,116,139,0.25),_5px_5px_10px_rgba(15,23,42,0.35)] transition-all duration-500 ${ 
                            ...
                            <div className="flex items-center justify-between mb-4">
                              ...
                              <div className={`flex items-center gap-1 text-sm ${ (currentKpiData[3]?.value && parseFloat(currentKpiData[3].value.replace(/[^0-9.]/g, '')) > 0) ? 'text-green-300' : 'text-red-300'}`}> {/* Example logic for trend */}
                                <TrendingUp className={`w-4 h-4 ${ (currentKpiData[3]?.value && parseFloat(currentKpiData[3].value.replace(/[^0-9.]/g, '')) < 0) ? 'rotate-180' : ''} transition-transform duration-500`} />
                                <span>{(currentKpiData[3]?.change) || '+0%'}</span>
                              </div>
                            </div>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                          ...
                          <div className="bg-slate-700 rounded-lg overflow-hidden shadow-[-5px_-5px_10px_rgba(100,116,139,0.25),_5px_5px_10px_rgba(15,23,42,0.35)] relative group">
                             <Image 
                              src="https://images.unsplash.com/photo-1601039913996-d74e58095333?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3NDE5ODJ8MHwxfHNlYXJjaHwxMHx8Q29tbWVyY2lhbCUyMHNwYWNlfGVufDB8fHx8MTc1MDQ4NTYxMXww&ixlib=rb-4.1.0&q=80&w=1080"
            ...
            <div className="mt-20 w-full max-w-5xl mx-auto z-10 relative">
                <h3 className="text-center text-gray-600 dark:text-gray-400 text-sm font-medium mb-6 flex items-center justify-center gap-2">
                ...
                <div className="overflow-hidden py-4">
                  <div className="flex animate-scroll-companies whitespace-nowrap">
                    {[...trustedByCompanies, ...trustedByCompanies].map((company, index) => (
                      <span key={index} className="text-gray-500 dark:text-gray-400 text-lg hover:text-gray-700 dark:hover:text-gray-200 transition-colors flex-shrink-0 mx-6 md:mx-10 lg:mx-16">
                        {company}
                      </span>
                    ))}
                  </div>
